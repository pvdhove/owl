...
module Make (Ord : Map.OrderedType) : sig
  ...
  (**
``mem k m`` returns true is the multimap ``m`` contains at least one binding for
``k``, false otherwise.
   *)
...
binding (they can be restored by calling ``remove k m``).
  ...
  (**
``remove k v m`` returns a multimap with the same bindings as ``m``, except for
the binding of ``k``: the last value that was bound to it is removed. If there
is no binding for ``k`` in ``m``, raises `Not_found`.
   *)

  val max_binding : 'a t -> key * 'a
  (**
``max_binding m`` returns the greatest binding in ``m``. Raises ``Not_found`` if
``m`` is empty.
   *)
